...
class GlobalEngagementScoreNew:
    ...
    def __init__(self, kpi_data: dict, seuil_minimal: float = 0.50):
        ...
        self._average_semantic_score: float = kpi_data["data"]["successful_posts"][
            "global_body_kpi"
    ...
    def __calculate_title_features_scores(
        ...
        target_subjectivity = self._target_kpi["global_title_kpi"][
            "average_title_subjectivity"
        ]
        ...
        polarity_score = self.proximity_score(
            draft_polarity, target_polarity, max_polarity_range
        )
        subjectivity_score = self.proximity_score(
            draft_subjectivity, target_subjectivity, max_subjectivity_range
        )
    ...
    def __calculate_body_features_scores(
        ...
        # STEP 1: Retrieve target and draft values
        target_length = self._target_kpi["global_body_kpi"]["words_and_sentences"][
            "average_word_count"
        ]
        target_polarity = self._target_kpi["global_body_kpi"][
            "polarity_and_readability_subjectivity"
        ]["average_polarity"]
        target_subjectivity = self._target_kpi["global_body_kpi"][
            "polarity_and_readability_subjectivity"
        ]["average_subjectivity"]
        target_readability = self._target_kpi["global_body_kpi"][
            "polarity_and_readability_subjectivity"
        ]["average_readability_score"]

        draft_length = self._draft_kpi["body_kpi"]["words_and_sentences"][
            "average_word_count"
        ]
        draft_polarity = self._draft_kpi["body_kpi"][
            "polarity_and_readability_subjectivity"
        ]["average_polarity"]
        draft_subjectivity = self._draft_kpi["body_kpi"][
            "polarity_and_readability_subjectivity"
        ]["average_subjectivity"]
        draft_readability = self._draft_kpi["body_kpi"][
            "polarity_and_readability_subjectivity"
        ]["average_readability_score"]
        ...
        polarity_score = self.proximity_score(
            draft_polarity, target_polarity, 2.0
        )  # polarité entre -1 et 1
        subjectivity_score = self.proximity_score(
            draft_subjectivity, target_subjectivity, 1.0
        )  # subjectivité entre 0 et 1
        readability_score = self.log_readability_score(
            draft_readability, target_readability
        )
    ...
    def __calculate_substance_features_scores(self) -> float:
        ...
        target_most_used_words = self._target_kpi["global_body_kpi"][
            "words_and_sentences"
        ]["most_used_words"]
        ...
        if number_of_target_words > 0:
            ...
            substance_score = (
                np.log1p(number_of_common_words) / np.log1p(number_of_target_words) * 10
            )

            # STEP 5 : Generate advice based on substance features
            if substance_score < 6:
                advice = [
                    "Substance: Consider incorporating more relevant keywords and phrases that are commonly used in successful posts to enhance the substance of your content."
                ]
    ...
    def generate_title_advice(
        ...
        if draft_length < target_length * 0.8:
            advice_list.append(
                "Title (Length): Consider increasing the title length to better capture attention."
            )
        elif draft_length > target_length * 1.2:
            advice_list.append(
                "Title (Length): Consider shortening the title to make it more concise and impactful."
            )

        # Advice on polarity
        if draft_polarity < target_polarity - 0.2:
            advice_list.append(
                "Title (Polarity): Try to make the title more positive to engage readers for example by highlighting benefits or solutions."
            )
        elif draft_polarity > target_polarity + 0.2:
            advice_list.append(
                "Title (Polarity): Try to make the title more neutral or balanced to avoid alienating readers."
            )

        # Advice on subjectivity
        if draft_subjectivity < target_subjectivity - 0.2:
            advice_list.append(
                "Title (Subjectivity): Consider adding more personal or emotional elements to the title to connect with readers."
            )
        elif draft_subjectivity > target_subjectivity + 0.2:
            advice_list.append(
                "Title (Subjectivity): Consider making the title more objective and fact-based to enhance credibility."
            )
    ...
    def generate_body_advice(
        ...
        if draft_length < target_length * 0.8:
            advice_list.append(
                "Body (Length): Consider increasing the body length to provide more comprehensive information."
            )
        elif draft_length > target_length * 1.2:
            advice_list.append(
                "Body (Length): Consider shortening the body to maintain reader engagement and focus."
            )

        # Advice on polarity
        if draft_polarity < target_polarity - 0.2:
            advice_list.append(
                "Body (Polarity): Try to make the body more positive to engage readers for example by highlighting benefits or solutions."
            )
        elif draft_polarity > target_polarity + 0.2:
            advice_list.append(
                "Body (Polarity): Try to make the body more neutral or balanced to avoid alienating readers."
            )

        # Advice on subjectivity
        if draft_subjectivity < target_subjectivity - 0.2:
            advice_list.append(
                "Body (Subjectivity): Consider adding more personal or emotional elements to the body to connect with readers."
            )
        elif draft_subjectivity > target_subjectivity + 0.2:
            advice_list.append(
                "Body (Subjectivity): Consider making the body more objective and fact-based to enhance credibility."
            )
        # Advice on readability
        if draft_readability < target_readability - 5:
            advice_list.append(
                "Body (Readability): Work on improving the readability of the body by using simpler language and shorter sentences."
            )
        elif draft_readability > target_readability + 5:
            advice_list.append(
                "Body (Readability): Consider using more complex language and varied sentence structures to enhance engagement."
            )

        return advice_list

    def generate_semantic_advice(self, semantic_score: float) -> list[str]:
        """
        ...
        if semantic_score < 0.7:
            advice_list.append(
                "Semantic Similarity: Consider aligning the content more closely with successful posts by incorporating similar themes and language."
            )
    ...
    def calculate_global_score(
        ...
        substance_score, substance_advice_list = (
            self.__calculate_substance_features_scores()
        )

        # STEP 4 : CALCULATE SEMANTIC FEATURES SCORE
        semantic_score, semantic_advice_list = (
            self.__calculate_semantic_features_scores()
        )
        ...
        all_advices = (
            advice_list
            + body_advice_list
            + substance_advice_list
            + semantic_advice_list
        )
