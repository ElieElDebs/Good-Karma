...
class DynamicGESCalculator:
    ...
    def __init__(self, full_kpi_data: dict):
        ...
        try:
            ...
            raise ValueError("La structure de données JSON est incorrecte. Les clés 'data', 'draft_post' ou 'successful_posts' sont manquantes.")
        

        # CONSTANT
        self.S_MIN = 0.7                      # Seuil minimal de similarité pour F_Sémantique.
        self.LENGTH_MIN_THRESHOLD = 100       # Mots minimum pour commencer à avoir un score > 0.
        ...
        self.W_LEXICAL = 1.5
        self.W_TOTAL = self.W_TITLE + self.W_READABILITY + self.W_LENGTH + self.W_SEMANTIC
        self.W_TOTAL_STRUCT = self.W_TITLE + self.W_READABILITY + self.W_LENGTH + self.W_LEXICAL
        self.CRITICAL_ADVICE_THRESHOLD = 5.0  # Seuil pour générer des conseils stratégiques.
        ...
        self.TARGET_READABILITY = self._get_target_kpi_value(
            successful_posts_kpi, ['global_body_kpi', 'polarity_and_readability_subjectivity', 'average_readability_score'], default_value=71.52
        )
        # TARGET_WORD_COUNT (global_body_kpi > word_count)
        self.TARGET_WORD_COUNT = self._get_target_kpi_value(
            successful_posts_kpi, ['global_body_kpi', 'words_and_sentences', 'average_word_count'], default_value=323.7
        )
        ...
        self.LENGTH_SLOPE_DIVISOR = length_range / score_range if score_range != 0 else 1.0
        ...
        try:
            self.MOST_USED_WORDS = [
                w for w, _ in successful_posts_kpi.get('global_body_kpi', {})
                    .get('words_and_sentences', {}).get('most_used_words', [])[:20]
            ]
        except Exception:
            pass
    ...
    def _calculate_factors(self, semantic_similarity_avg: float) -> dict:
        ...
        draft_polarity = self._get_draft_kpi_value(['title_kpi', 'average_title_polarity'], default_value=0.0)
        draft_readability = self._get_draft_kpi_value(['body_kpi', 'polarity_and_readability_subjectivity', 'average_readability_score'], default_value=self.TARGET_READABILITY)
        draft_word_count = self._get_draft_kpi_value(['body_kpi', 'words_and_sentences', 'average_word_count'], default_value=0)
        ...
        if draft_readability >= self.TARGET_READABILITY:
            ...
            F_Lisibilité = np.clip(10 - np.abs(draft_readability - self.TARGET_READABILITY) / 2.5, 0, 10)
        ...
        if draft_word_count < self.LENGTH_MIN_THRESHOLD:
            ...
            norm = (draft_word_count - self.LENGTH_MIN_THRESHOLD) / (max_wc - self.LENGTH_MIN_THRESHOLD)
            norm = max(0, norm)
            # log_base(1 + norm * (base-1)) to get 0 at min, 1 at target
            log_score = np.log1p(norm * (self.LENGTH_LOG_BASE - 1)) / np.log(self.LENGTH_LOG_BASE)
            F_Longueur = np.clip(self.LENGTH_MIN_SCORE + (10 - self.LENGTH_MIN_SCORE) * log_score, 0, 10)
    ...
    def generate_advice_list(self, results: dict) -> list[str]:
        ...
        confidence_multiplier = semantic_similarity_avg ** 1.5 if semantic_similarity_avg > 0 else 0.0
        ...
        if score < self.CRITICAL_ADVICE_THRESHOLD:
            draft_polarity = self._get_draft_kpi_value(['title_kpi', 'average_title_polarity'])
            advice_list.append(
        ...
        if score < self.CRITICAL_ADVICE_THRESHOLD:
            draft_word_count = self._get_draft_kpi_value(['body_kpi', 'words_and_sentences', 'average_word_count'])
        ...
        draft_readability = self._get_draft_kpi_value(['body_kpi', 'polarity_and_readability_subjectivity', 'average_readability_score'])
        ...
        if not advice_list:
            advice_list.append("The core of your post is strong. Focus only on minor improvements.")
