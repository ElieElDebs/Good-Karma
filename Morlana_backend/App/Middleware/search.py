...
from App.Utils.kpi import calculate_body_kpi, calculate_titles_kpi, calculate_GES, calculate_GES_new
from App.Utils.utils import preprocess_text
from qdrant_client.http.models import Filter, FieldCondition, MatchValue

def _search_similar_posts_new(query: str, top_k: int = 5, subreddits: list = None, min_score: float = None) ->  dict[str, pd.DataFrame]:
    """
    ...
    for subreddit in subreddits:
        ...
        results = qdrant.search_embeddings(
            collection_name="morlana_collection", query=query, top_k=top_k, filter=successful_filter
        )
...
def calculcate_posts_kpi_new (title:str, body:str, top_k:int, subreddits: list = None, min_score:float=None) -> dict[str , any] :
    """
    ...
    for subreddit, posts in posts_by_subreddit.items():
        posts_by_subreddit_api[subreddit] = posts.to_dict(orient="records") if not posts.empty else []
        ...
        b_global_kpis, posts_with_kpi = calculate_body_kpi(posts, text_column="preprocessed_text")
...
def _search_similar_posts(query: str, top_k: int = 5, subreddits: list = None, min_score: float = None) -> tuple[dict, pd.DataFrame]:
    """
    ...
    results_unsuccessful = qdrant.search_embeddings(
        collection_name="morlana_collection", query=query, top_k=top_k, filter=unsuccessful_filter
    )
...
def calculcate_posts_kpi (title:str, body:str, top_k:int, min_score:float=None) -> dict[str , any] :
    """
    ...
    successful_title_posts_kpi = calculate_titles_kpi(s_posts, text_column="preprocessed_title")
    s_b_global_kpis, s_posts_with_kpi = calculate_body_kpi(s_posts, text_column="preprocessed_text")
    ...
    ges_result, advice_list = calculate_GES({
        "data": api_response
    })
